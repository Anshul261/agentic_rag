...
export function AgentTrace({ steps, currentProgress, isLoading = false, compact = false }: AgentTraceProps) {
  ...
  return (
    <div className={cn(
      "border border-border rounded-sm bg-card/50",
      compact ? "p-2" : "p-3"
    )}>
      ...
      {currentProgress && (
        <div className="mb-2 px-2 py-1.5 bg-primary/5 border border-primary/20 rounded-sm text-xs text-foreground flex items-center gap-2 font-mono">
        ...
        <div className="space-y-1.5">
          {displaySteps.map((step, index) => (
            <div
              key={index}
              className={cn(
                "flex items-start gap-2 text-xs transition-all font-mono",
                compact && "py-0.5"
              )}
            >
              <div className="mt-0.5 flex-shrink-0">
                {step.status === 'completed' ? (
                  ...
                  <div className={cn(
                    ...
                    step.type === 'agent_start' ? "bg-primary" :
                    step.type === 'agent_end' ? "bg-muted-foreground" :
                    "bg-primary/60"
                  )} />
                )}
              </div>
      ...
      {compact && steps.length > 3 && (
        <div className="text-center mt-1.5 pt-1.5 border-t border-border">
